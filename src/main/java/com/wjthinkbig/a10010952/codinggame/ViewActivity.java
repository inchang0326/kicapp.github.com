...
public class ViewActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Log.d("inchang", this.getClass().getName().toString());
    ...
    private char[][] getAlphaMap(ArrayDeque<Character> queue) {
        ...
        while (queue.size() > 0) {
            ...
            if (isVisited[nextR][nextC]) {
                ...
                // (4-1) 다음 위치가 방문했던 곳이라면(false) 현재 위치(currR, currC)에서 4방면을 확인
            } else {
                flag = false;
                for (int i = 0; i < 4; i++) {
                    ...
                    if (isVisited[nextR][nextC]) {
                        ...
                        // (4-2) 현재 위치에서 주변에 하나라도 갈 곳이 있으면 flag를 true로 설정
                        flag = true;
                ...
                // (5) 한 곳으로라도 갈 곳이 없다면 이전 위치를 스택에서 팝
                if (!flag) {

                    // (6) 주어진 단어의 알파벳이 모두 채워지지 않았는데 길이 막혔을 경우, 맵 재배치
                    ...
                    // (7) 주어진 단어의 알파벳이 모두 채워졌는데 길이 막혔을 경우, 이전 위치로 이동
                    else {
                        Point p = stack.pop();
                        currR = p.getRow();
                        currC = p.getCol();
