...
public class ViewActivity extends Activity {
    ...
    /*
       ...
       1) 정답인 코딩 블록들을 저장함
       2) AsyncTask를 상속한 클래스로 정답인 코딩 블록들을 매개함
     */
    private Queue<Arrow> m_queueForMovingTask = new ArrayDeque<>();
    /* m_stackForAnswers
       1) 정답인 코딩 블록들을 저장함
    ...
    private int m_moveToMarginLeft = 0,// UI x축 이동량
            ...
            m_winCnt = 0;// 이긴 횟수
    private boolean m_repeatAvaiable = true,// 반복 블록이 가능한지 아닌지에 대한 플래그
            m_backAvailable = true,// back 버튼이 가능한지 아닌지에 대한 플래그
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        m_run = findViewById(R.id.run);
        m_codingStation = findViewById(R.id.codingStation);
        ...
        // 레이아웃 모음
        m_alphaMapContainer = findViewById(R.id.tableLayout);
        m_dingcoContainer = findViewById(R.id.dingcocontainer);
    ...
    private void init(int isClear) {
        switch (isClear) {
            // clear 버튼이 아닌 init()은 위치를 랜덤으로 다시 받는다.
                    ...
                    switch (view.getId()) {
                        case R.id.up:
                            ...
                            try {
                                ...
                                m_stackForCodingStation.add(new GridViewItem(getResources().getDrawable(R.drawable.up)));
                                m_codingStation.setAdapter(m_codingStationAdapter);
                                m_repeatAvaiable = true;
                            } catch (Exception e) {
                        ...
                        case R.id.down:
                            ...
                            try {
                                ...
                                m_stackForCodingStation.add(new GridViewItem(getResources().getDrawable(R.drawable.down)));
                                m_codingStation.setAdapter(m_codingStationAdapter);
                                m_repeatAvaiable = true;
                            } catch (Exception e) {
                                m_toastTextView.setText("블록을 다시 넣어주세요.");
                        ...
                        case R.id.left:
                            ...
                            try {
                                ...
                                m_stackForCodingStation.add(new GridViewItem(getResources().getDrawable(R.drawable.left)));
                                m_codingStation.setAdapter(m_codingStationAdapter);
                                m_repeatAvaiable = true;
                            } catch (Exception e) {
                                m_toastTextView.setText("블록을 다시 넣어주세요.");
                        ...
                        case R.id.right:
                            ...
                            try {
                                ...
                                m_stackForCodingStation.add(new GridViewItem(getResources().getDrawable(R.drawable.right)));
                                m_codingStation.setAdapter(m_codingStationAdapter);
                                m_repeatAvaiable = true;
                            } catch (Exception e) {
                                m_toastTextView.setText("블록을 다시 넣어주세요.");
            ...
            switch (v.getId()) {
                ...
                case R.id.run: {
                    ...
                    if (m_correctAlphaCnt == m_voca.length()) {
                        ...
                        init(CLEAR_BTN_NOT_CLICKED);// 게임 정보 초기화
                        generateMap();// 맵 생성
                        m_winCnt++;// 승리 수 증가
                        if (m_winCnt > 0) {
